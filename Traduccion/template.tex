...
\begin{abstract}
  In color ordering, the lexicographical ordering and its variants are the most used methods and an usual problem is the ``a priori'' definition of the most important color component and this arbitrary decision results on most comparations been decided by the first components.
  This paper proposes a new ordering criteria, which assign a weight to every component in accordance with the metrics applied to it, with this we pursue to avoid arbitrary definitions and build the ordering based on image-based information.
  Applications used for validation of the proposal are: image filtering, contrast enhacement and textures characterization for later classification. The results using the proposed ordering in different applications are better in most cases compared to different ordering methods of the state of the art.
\end{abstract}
...
This work presents a new ordering for the RBG color space based on the metrics associated to each component. The proposed ordering is compared with the state of art orderings on noise reduction, contrast enhancement and texture characterization.
...
RGB colors are reduced to a scalar value. For this purpose, is first defined a transformation $T:\mathbb{Z}^3 \rightarrow \mathbb{R}$ and then ordered te colors with respect to the order of his scalar projection in $\mathbb{Z}^3$ by $T$. \textbf{NE} %TODO
...
In the Figure \ref{fig:ruido}(a) we can see an image that is contaminated with Gaussian noise (\ref{fig:ruido}(b)), speckle noise (\ref{fig:ruido}(c)), with salt and pepper noise (\ref{fig:ruido}(d)). 
...
\begin{table}[!h]
	\centering
	\caption{Gaussian noise. MAE sorted by $\sigma^2$}
	\pgfkeys{/pgf/number format/precision=4}
	\pgfplotstabletypeset[
	col sep=tab,
	%header=has colnames,
	columns = {[index]0, [index]10},
	columns/P/.style = {column name = Sum},
	columns/p/.style = {column name = Filter},
...
\begin{figure*}[htbp]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Gaussian noise. MAE by $\sigma^2$}
...
Figure \ref{fig:salt_and_pepper_mae} shows the trend curves of the filters applied to images contaminated with salt and pepper noise. In this case the filter that obtained the lowest overall average and whose curve is placed below all others was ED. The MAX, SMO, VAR and MEAN filters showed better performances after ED. Table \ref{tabla:salt_and_pepper_mae} contains the total sum of the points on the curve trend by sorting method.
...
\begin{equation} %TODO Significado de ``CM''
